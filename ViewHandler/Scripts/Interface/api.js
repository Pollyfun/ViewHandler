...
var ViewHandler = function () {
   ...
   function createView(containerId, configName, viewHandlerPath, callback) { // containerId first. mandatory here.
      ...
      if (typeof eventListeners[containerId] === 'undefined') {
         eventListeners[containerId] = "";
         document.body.addEventListener('click', function () {
            clickedOnParentForm(containerId);   // containerId required when > 1 viewhandler.
         });
      }
      else {
         console.info('already defined: ' + containerId);
      }
   }
   var eventListeners = [];   // list of containers ids. to make sure click event is only added once for each viewhandler
